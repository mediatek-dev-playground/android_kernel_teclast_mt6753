...
#define UI_SOC_CHARGING_CALI_TIME   70//140//110//60    //secs
#define UI_SOC_CHARGING_CALI_INIT_TIME   10   //secs
#define UI_SOC_CHARGING_CALI_CHK_CONTINUE_CUR_TIMES   3
#define UI_SOC_CHARGING_CALI_TERMINATION_CURR    250//mA
#define UI_SOC_CHARGING_CALI_MAX_CURRENT  UI_SOC_CHARGING_CALI_TERMINATION_CURR+100  //mA
#define UI_SOC_CHARGING_CALI_MAX_PERCENT   99  //99%
#define UI_SOC_CHARGING_CALI_TIMER_INIT   (UI_SOC_CHARGING_CALI_INIT_TIME / BAT_TASK_PERIOD)
static void mt_battery_99Percent_tracking_charging_check(void)
{
	kal_uint32 bat_vol = 0,icharging = 0;
	kal_uint32 cust_sync_time = UI_SOC_CHARGING_CALI_TIME;
	static kal_uint32 currindex = 0;
	static kal_uint32 timer_counter = UI_SOC_CHARGING_CALI_TIMER_INIT; 
	static kal_bool b_cali_condition = FALSE;
	kal_bool b_first_time_cali = FALSE;
	bat_vol = battery_meter_get_battery_voltage(KAL_TRUE);
	battery_log(BAT_LOG_CRTI,"charging_check:UI_SOC:%d SOC:%d bat_vol:%d bat_full:%d\n",
	            BMT_status.UI_SOC,BMT_status.SOC,bat_vol, BMT_status.bat_full);
    if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR) && 
        ...
        if(icharging <= UI_SOC_CHARGING_CALI_MAX_CURRENT) 
        {
            if(b_cali_condition == FALSE)
            {
        	  currindex++;
        	  if(currindex == UI_SOC_CHARGING_CALI_CHK_CONTINUE_CUR_TIMES)
        	  {
        	      b_cali_condition = TRUE;
        	      b_first_time_cali = TRUE;
        	      currindex = 0;
        	      battery_log(BAT_LOG_CRTI,"Start UI_SOC cali!\n");
        	  }
        	}
        }
        else
        ...
        if(b_cali_condition)
		{
			if((timer_counter >= (cust_sync_time/BAT_TASK_PERIOD)) || (b_first_time_cali == TRUE))
			{
			    b_first_time_cali = FALSE;
				if(BMT_status.UI_SOC < UI_SOC_CHARGING_CALI_MAX_PERCENT)//99
				{
					cali_ui_soc ++;	
					BMT_status.UI_SOC ++;
					battery_log(BAT_LOG_CRTI,"charging_check:[cali_ui_soc++:%d]UI_SOC[%d]SOC[%d]\n",
					                    cali_ui_soc,BMT_status.UI_SOC,BMT_status.SOC);
				}
				timer_counter = UI_SOC_CHARGING_CALI_TIMER_INIT;// 1
			}
			else
			{
				timer_counter ++ ;
				battery_log(BAT_LOG_CRTI,"charging_check:timer_counter ++[%d]\n",timer_counter);
			}
		}  
    }
    if(BMT_status.charger_exist == KAL_FALSE || BMT_status.UI_SOC == 100)
    {
        cali_ui_soc = 0;
        currindex = 0;
        b_cali_condition = FALSE;        
        b_first_time_cali = FALSE;
        battery_log(BAT_LOG_CRTI,"charging_check:clear flag\n");
    }
}
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if(chr_wake_up_bat==KAL_FALSE)
		{
			...
			{
			BMT_status.UI_SOC--;
			battery_log(BAT_LOG_CRTI, "charging_check:@1UI_SOC--:%d\n",BMT_status.UI_SOC);
			}
			timer_counter = 0;
			} 
#ifdef FG_BAT_INT
			else if(fg_wake_up_bat==KAL_TRUE)
			{
			    if(cali_ui_soc == 0){
				BMT_status.UI_SOC--;	
				battery_log(BAT_LOG_CRTI, "charging_check:@2UI_SOC--:%d\n",BMT_status.UI_SOC);
				}
			}
#endif //#ifdef FG_BAT_INT			
			else 
...
static void battery_update(struct battery_data *bat_data)
{
	...
	mt_battery_99Percent_tracking_charging_check();

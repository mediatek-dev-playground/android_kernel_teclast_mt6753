...
#ifdef CAM_CAL_DEBUG
#define CAM_CALDB(fmt, arg...)  xlog_printk(ANDROID_LOG_DEBUG, "[S5K3L2XXTRULY_EEPROM]" , fmt, ##arg)//printk
#else
#define CAM_CALDB(x,...)
#endif
...
extern u8 OTPData[];	//add by lijin 2015.3.19
int otp_flag = 0;
...
int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    ...
    if (i4RetValue != 2)
    {
        CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] I2C send read address failed!! \n");
        return -1;
    }
	
    i4RetValue = i2c_master_recv(g_pstI2Cclient, (char *)a_puBuff, ui4_length);
	CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
	
    if (i4RetValue != ui4_length)
    {
        CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] I2C read data failed!! \n");
        return -1;
    } 
...
//add by lijin 2015.3.19
int Read_LSC_Data(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int readbuff;
   int RetValue;
   int i;
    if(OTPData[0]==0xff)//(otp_flag)
    {
       for(i=0; i<ui4_length-1; i++)
        pinputdata[i] = OTPData[i+1];
	CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][Read_LSC_Data] Read lsc data form OTPData[%d]=0x%x \n", i, pinputdata[i]);
    }
    else
    {
	for(i=0; i<ui4_length; i++)
        {
	       RetValue = iReadCAM_CAL((u16)ui4_offset+i, 1, readbuff);
	       if (RetValue != 0)
	       {
		    CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] [Read_LSC_Data]I2C iReadData failed!! \n");
		    return -1;
	       }   
              *(pinputdata + i) = readbuff;
	      CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][Read_LSC_Data] Read lsc data[%d]=0x%x \n", i, pinputdata[i]);
        }
   }
   otp_flag = 0;
   return 0;
...
/*static*/ int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   if(0)//(ui4_length == 1868)
   {
   ...
   else if(ui4_length ==8)
    {
	    ...
	    u8 readbuff, i;
			...
			for(i=0;i<ui4_length;i++)
			{
	    iReadCAM_CAL((u16)u4CurrentOffset, 8, readbuff);
			*(pBuff+i)=readbuff;
		  CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][iReadData] read awb data[i]=0x%x\n",i,pBuff[i]);
			}
	...
	else if(ui4_length ==4)
    {
	    ...
	    u8 readbuff, i;
			...
			for(i=0;i<ui4_length;i++)
			{
	        iReadCAM_CAL((u16)u4CurrentOffset, 4, readbuff);
					*(pBuff+i)=readbuff;
		      CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][iReadData] read af data[i]=0x%x\n",i,pBuff[i]);
			}
   ...
   do 
   {
	   ...
	   CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL][iReadData] Read 0x%x=0x%x \n", u4CurrentOffset, pBuff[0]);
       if (i4RetValue != 0)
       {
            CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] I2C iReadData failed!! \n");
            return -1;
       }           
       u4IncOffset++;
       i4ResidueDataLength --;
...
#endif
{
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     //CAM_CALDB("[S24CAM_CAL] init Working buffer address 0x%llx  command is 0x%llx\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] Read CMD \n");
	    ...
	    otp_flag = 1;	//add by lijin 2015.3.20
            ...
            CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] offset 0x%x \n", ptempbuf->u4Offset);
            CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] length %d \n", ptempbuf->u4Length);
            //CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] Before read Working buffer address 0x%llx \n", (u32)pWorkingBuff);
	    #if 1		//add by lijin 2015.3.19
            ...
            CAM_CALDB("[S5K3L2XXTRULY_CAM_CALL] After read Working buffer data  0x%8x \n", *pWorkingBuff);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        //CAM_CALDB("[S24CAM_CAL] to user  Working buffer address 0x%llx \n", (u32)pWorkingBuff);
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = S5K3L2XXTRULYOTP_DEVICE_ID >> 1;

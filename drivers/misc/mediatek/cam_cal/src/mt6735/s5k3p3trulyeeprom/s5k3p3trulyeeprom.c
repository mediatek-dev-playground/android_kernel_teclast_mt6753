...
#ifdef CAM_CAL_DEBUG
#define CAM_CALDB(fmt, arg...)  xlog_printk(ANDROID_LOG_DEBUG, "[S5K3P3TRULY_EEPROM]" , fmt, ##arg)
#else
#define CAM_CALDB(x,...)
#endif
#define CAM_CALERR(fmt, arg...)  xlog_printk(ANDROID_LOG_ERROR, "[S5K3P3TRULY_EEPROM]" , fmt, ##arg)//printk
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "CAM_CAL_DRV_3P3"
#define CAM_CAL_I2C_GROUP_ID 0
/*******************************************************************************
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_3p3_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0x68>>1)};
...
extern u8 S5K3P3_LSCData[];
extern u8 S5K3P3_AFData[];
extern u8 S5K3P3_AWBData[];

extern int s5k3p3_prepare_lsc_ok;
extern int s5k3p3_prepare_af_ok;
extern int s5k3p3_prepare_awb_ok;

extern bool prepare_3p3_lsc_eeprom(void);
extern bool prepare_3p3_af_eeprom(void);
extern bool prepare_3p3_awb_eeprom(void);
...
static int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
	...
	//CAM_CALDB("[S5K3P3TRULY_CAM_CALL][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
...
#endif
{
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     //CAM_CALDB("[S5K3P3TRULY_CAM_CALL] init Working buffer address 0x%llx  command is 0x%llx\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
		...
		case CAM_CALIOC_G_READ:
			...
			*/
			...
			if (ptempbuf->u4Offset==0x20 && ptempbuf->u4Length==1868) {
				while (s5k3p3_prepare_lsc_ok!=1 && retry_t>0) {
					prepare_3p3_lsc_eeprom();
					retry_t--;
				}
				if (retry_t != 0) {
					CAM_CALDB("prepare lsc ok, retry_time=%d\n",retry_t);
					memcpy(pWorkingBuff, S5K3P3_LSCData, 1868);
					i4RetValue = 0;
				} else {
					i4RetValue = -EPERM;
			...
			} else if (ptempbuf->u4Offset==0x12 && ptempbuf->u4Length==8) {
				retry_t = 3;
				while (s5k3p3_prepare_awb_ok!=1 && retry_t>0) {
					prepare_3p3_awb_eeprom();
					retry_t--;
				}
				if (retry_t != 0) {
					CAM_CALDB("prepare awb ok, retry_time=%d\n",retry_t);
					memcpy(pWorkingBuff, S5K3P3_AWBData, 8);
					i4RetValue = 0;
				} else {
					i4RetValue = -EPERM;
					CAM_CALDB("prepare awb failed, retry_time=%d\n",retry_t);
					break;
				}
			} else if (ptempbuf->u4Offset==0x1a && ptempbuf->u4Length==4) {
				retry_t = 3;
				while (s5k3p3_prepare_af_ok!=1 && retry_t>0) {
					prepare_3p3_af_eeprom();
					retry_t--;
				}
				if (retry_t != 0) {
					CAM_CALDB("prepare af ok, retry_time=%d\n",retry_t);
					memcpy(pWorkingBuff, S5K3P3_AFData, 4);
					i4RetValue = 0;
				} else {
					i4RetValue = -EPERM;
					CAM_CALDB("prepare af failed, retry_time=%d\n",retry_t);
					break;
				}
			} else if (ptempbuf->u4Offset==0x1a && ptempbuf->u4Length==2) {
				retry_t = 3;
				while (s5k3p3_prepare_af_ok!=1 && retry_t>0) {
					prepare_3p3_af_eeprom();
					retry_t--;
				...
				if (retry_t != 0) {
					CAM_CALDB("prepare af ok, retry_time=%d\n",retry_t);
					memcpy(pWorkingBuff, S5K3P3_AFData, 2);
					i4RetValue = 0;
				} else {
					i4RetValue = -EPERM;
					CAM_CALDB("prepare af failed, retry_time=%d\n",retry_t);
			...
			} else if (ptempbuf->u4Offset==0x1c && ptempbuf->u4Length==2) {
				retry_t = 3;
				while (s5k3p3_prepare_af_ok!=1 && retry_t>0) {
					prepare_3p3_af_eeprom();
					retry_t--;
				...
				if (retry_t != 0) {
					CAM_CALDB("prepare af ok, retry_time=%d\n",retry_t);
					memcpy(pWorkingBuff, &S5K3P3_AFData[2], 2);
					i4RetValue = 0;
				} else {
					i4RetValue = -EPERM;
					CAM_CALDB("prepare af failed, retry_time=%d\n",retry_t);
			...
			} else if (ptempbuf->u4Offset==0x04 && ptempbuf->u4Length==1) {
				// trunly mid=0x02
				*pWorkingBuff = 2;
				i4RetValue = 0;
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        //CAM_CALDB("[S5K3P3TRULY_CAM_CALL] to user  Working buffer address 0x%llx \n", (u32)pWorkingBuff);
...
static int CAM_CAL_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    {
        CAM_CALDB("[S5K3P3TRULY_CAM_CALL] Fail to enable analog gain\n");
        return -EIO;
    }
#endif
    return 0;
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "CAM_CALdrv_3P3");
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = S5K3P3TRULYEEPROM_DEVICE_ID >> 1;
...
{
    i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_3p3_dev, 1);

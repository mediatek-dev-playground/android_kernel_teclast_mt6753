...
#ifdef MSENSOR_DATA_OPTIMIZE
#define MAX_TIMES 7//10//30//35
#endif
...
#define BMM050_I2C_ADDRESS                 0x12//0x10
		...
		BMM050_S16 *diff_z)
{
	...
	if (p_bmm050 == BMM050_NULL) {
		...
		comres |= bmm050api_read_mdataXYZ_s32(&mdata);
...
#endif
#ifdef MSENSOR_DATA_OPTIMIZE
    static int mx[MAX_TIMES]={0},my[MAX_TIMES]={0},mz[MAX_TIMES]={0};
    static int isfirsttime = 1;
    int totalmx =0, totalmy =0, totalmz = 0;
    int tempxyzstatus[4] = {0};
    int resultxyzstatus[4] = {0};
    int i=0;
    static int index = 0;
#endif    
...
#ifdef MSENSOR_DATA_OPTIMIZE
        memcpy(tempxyzstatus, buf+1, 4*sizeof(int));
        if(isfirsttime)
        {
            isfirsttime = 0;
            for(i=0; i< MAX_TIMES; i++)
            {
                mx[i] = tempxyzstatus[0];
                my[i] = tempxyzstatus[1];
                mz[i] = tempxyzstatus[2];
            }
            memcpy(sensor_data+4, tempxyzstatus, 4*sizeof(int));
        }
        else
        {
            mx[index] = tempxyzstatus[0];
            my[index] = tempxyzstatus[1];
            mz[index] = tempxyzstatus[2];
            index ++;
            if(index >(MAX_TIMES-1))
            {
                index = 0;
            }
            for(i=0; i< MAX_TIMES; i++)
            ...
            totalmx = totalmx/MAX_TIMES;
            totalmy = totalmy/MAX_TIMES;
            totalmz = totalmz/MAX_TIMES;
            //MSE_LOG("totalxyz:%d %d %d\n",totalmx,totalmy,totalmz);
            resultxyzstatus[0] = totalmx;
            resultxyzstatus[1] = totalmy;
            resultxyzstatus[2] = totalmz;
            resultxyzstatus[3] = tempxyzstatus[3];
            memcpy(sensor_data+4, resultxyzstatus, 4*sizeof(int));
        }
	    MSE_LOG("ECS_SaveData-2:%d %d %d %d\n",sensor_data[4],sensor_data[5],sensor_data[6],sensor_data[7]);        
#else
...
static int ECS_GetRawData(int data[3])
{
	...
	bmm050api_read_mdataXYZ_s32(&mdata);
	//data in uT

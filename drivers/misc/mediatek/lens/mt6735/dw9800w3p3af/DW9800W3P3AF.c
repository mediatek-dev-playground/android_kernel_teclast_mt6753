...
#define AF_DRVNAME "DW9800W3P3AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x18
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9800w3p3af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9800w3p3af"
...
static int inf_code= 200;
static int mac_code=800;

static unsigned short AF_convert(u16 position)
{
  if(mac_code<=inf_code)
    return 0;
  USHORT convert_code = (position*(mac_code-inf_code)/1023+inf_code);
  LOG_INF("convert_code %6d \n",convert_code);
  return convert_code;  
}
...
static int s4AF_WriteReg(u16 a_u2Data)
{
    ...
    char puSendCmd0[3] = {0x03, (char)(a_u2Data >> 8), (char)(a_u2Data & 0xff)};

    //LOG_INF("write %d \n", a_u2Data);
    g_pstAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd0, 3);
...
inline static int getAFInfo(__user stDW9800W3P3AF_MotorInfo * pstMotorInfo)
{
    stDW9800W3P3AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stDW9800W3P3AF_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stDW9800W3P3AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stDW9800W3P3AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stDW9800W3P3AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case DW9800W3P3AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDW9800W3P3AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case DW9800W3P3AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stDW9800W3P3AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case DW9800W3P3AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case DW9800W3P3AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case DW9800W3P3AFIOC_T_SETMACROPOS :
...
static int __init DW9800W3P3AF_i2C_init(void)
{
...
static void __exit DW9800W3P3AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(DW9800W3P3AF_i2C_init);
module_exit(DW9800W3P3AF_i2C_exit);

MODULE_DESCRIPTION("DW9800W3P3AF lens module driver");
MODULE_AUTHOR("Jane.zhangjiajuan <zhangjiajuan@malatamobile.com>");
MODULE_LICENSE("GPL");

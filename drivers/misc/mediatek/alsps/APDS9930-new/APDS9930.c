...
/*----------------------------------------------------------------------------*/
#define APS_TAG                  "[ALS/PS] APDS9930 "
#define APS_FUN(f)               printk(KERN_INFO APS_TAG"%s\n", __FUNCTION__)
#define APS_ERR(fmt, args...)    printk(KERN_ERR  APS_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define APS_LOG(fmt, args...)    printk(KERN_ERR APS_TAG fmt, ##args)
#define APS_DBG(fmt, args...)    printk(KERN_INFO APS_TAG fmt, ##args)
#ifdef AUTO_PS_CALIBRATION_ENABLE
#define PS_CALITAG     "APD9930-PS-CALI"
#define PS_CALILOG(fmt, args...)    printk(KERN_ERR PS_CALITAG fmt, ##args) 
#endif
...
#ifdef AUTO_PS_CALIBRATION_ENABLE
#define PS_SAMPLE_NOISE_NUM  5
#define PS_MAX_NOISE_VALUE  300//400
#define PS_MAX_VALUE		1023
#define PS_MIN_VALUE		0
int intr_ps_auto_calibration_flag = 0;

int APDS9930_ps_auto_calibration(struct i2c_client *client);
#endif

/*added for tp pocket mode:start*/
int apds9930_ps_enabled_for_tp  = 0;
int ps_thd_val_high_for_tp = 0;
int ps_thd_val_low_for_tp = 0;
/*added for tp pocket mode:end*/
...
int APDS9930_CMM_PPCOUNT_VALUE = 0x1A;//0x08;
int APDS9930_CMM_CONTROL_VALUE = 0x60;
int ZOOM_TIME = 39;
int APDS9930_CMM_MAGINITUDE_VALUE = 0x05;
	...
	if(enable)
		{
			...
			/*added for tp pocket mode:start*/
			apds9930_ps_enabled_for_tp = 1;			
			/*added for tp pocket mode:end*/
#ifdef AUTO_PS_CALIBRATION_ENABLE
            PS_CALILOG("APDS9930_enable_ps-ps auto calibration\n");
            if(intr_ps_auto_calibration_flag == 0)
                APDS9930_ps_auto_calibration(client);
            else
                PS_CALILOG("APDS9930_enable_ps-ps: already calibrated!\n");
#endif
		}
	else{
...
static int APDS9930_init_client(struct i2c_client *client)
{
	...
	databuf[0] = APDS9930_CMM_MAGINITUDE;    
	databuf[1] = APDS9930_CMM_MAGINITUDE_VALUE;
	res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if(res <= 0)
	{
		goto EXIT_ERR;
...
long APDS9930_read_ps(struct i2c_client *client, u16 *data)
{
	...
	//APS_LOG("%s,APDS9930_CMM_PDATA:%d, ps_cali:%d\n",__func__,temp_data,obj->ps_cali);
...
int APDS9930_ps_auto_calibration(struct i2c_client *client)
{
	...
	u16 temp_data = 0;
	u8 databuf[3];
	int firsttimegetps = 0;
	if(client == NULL)
	{
		PS_CALILOG("CLIENT CANN'T EQUL NULL\n");
		goto EXIT_ERR;
	}
	for(i=0;i<(PS_SAMPLE_NOISE_NUM*2); i++)
	{
	    if(firsttimegetps == 0)
	    {
	        firsttimegetps = 1;
	        ...
	        mdelay(20);	
		res = APDS9930_read_ps(client, &temp_data);	
		if(res != 0)
		{
		    PS_CALILOG("read ps fail!\n");
			continue;
		}
		//ingore ps value if offset>50	
        if(validnum >0)
        {
          	if(((temp_data - data_total/validnum) >50)||((temp_data - data_total/validnum) <-50))
		    //if(abs(temp_data - data_total/validnum) >50)
		    {
			    PS_CALILOG("invalid ps data, delete it:temp_data:%d!!\n",temp_data);
			    continue;
		    }  
        }
		if(validnum >= PS_SAMPLE_NOISE_NUM)
			break;
		validnum ++;
		data_total += temp_data;
		PS_CALILOG("valid ps data, acc(++) it:data_total:%d!!\n",data_total);        
	}
	if(validnum < PS_SAMPLE_NOISE_NUM)
	{
	    PS_CALILOG("No enough ps data count,cancel auto calibration!\n");
	    goto EXIT_ERR;
	}
	ps_average = (data_total ==0)?(0):(data_total/PS_SAMPLE_NOISE_NUM);
	PS_CALILOG("ps_average = %d,obj->ps_cali=%d\n", ps_average,obj->ps_cali);
	if(ps_average <= PS_MAX_NOISE_VALUE)
	{
	    threshold = ps_average + obj->hw->ps_threshold_high;
	    if(threshold >= PS_MAX_VALUE)
	        threshold = PS_MAX_VALUE-10;					
	    atomic_set(&obj->ps_thd_val_high, threshold);
	    /*added for tp pocket mode:start*/	    
		ps_thd_val_high_for_tp = threshold;
		/*added for tp pocket mode:end*/
	    threshold = ps_average + obj->hw->ps_threshold_low;
	    if(threshold >= PS_MAX_VALUE)
	             threshold = PS_MAX_VALUE-20;						
	    atomic_set(&obj->ps_thd_val_low, threshold);
	    /*added for tp pocket mode:start*/		
	    ps_thd_val_low_for_tp = threshold;
	    /*added for tp pocket mode:end*/
	    databuf[0] = APDS9930_CMM_INT_LOW_THD_LOW;	
	    databuf[1] = (u8)((atomic_read(&obj->ps_thd_val_low)) & 0x00FF);
	    res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	    ...
	    PS_CALILOG("ps_thd_val_low = %d, ps_thd_val_high = %d\n", atomic_read(&obj->ps_thd_val_low),atomic_read(&obj->ps_thd_val_high));			 
	}
	else
	{
	    PS_CALILOG("ps_average=%d(>[%d:noise]) is too high,disable auto calibated.\n",ps_average,PS_MAX_NOISE_VALUE);
	}
	PS_CALILOG("ps_thd_val_low = %d, ps_thd_val_high = %d\n", atomic_read(&obj->ps_thd_val_low),atomic_read(&obj->ps_thd_val_high));			 
	return 0;
EXIT_ERR:
    PS_CALILOG("fail to ps auto calibration!\n");
    return -1;
}

#endif
/*added for tp pocket mode:start*/
void APDS9930_enable_ps_for_tp(int enable)
{
    static int ps_need_disable_for_tp = 0;
    if(APDS9930_i2c_client != NULL)
    {
        if(enable  == 1)
        {
            if(apds9930_ps_enabled_for_tp == 0)
            {
	            APS_LOG("pockets open ps:%d\n",enable);
	            ps_need_disable_for_tp = 1;
	            APDS9930_enable_ps(APDS9930_i2c_client, enable);
	            msleep(200);
            }
            else
            {
                APS_LOG("pockets already open\n");
            }
        }
        else if(enable  == 0)
        {
            if(ps_need_disable_for_tp == 1)
            {
                APS_LOG("pockets close ps:%d\n",enable);
                ps_need_disable_for_tp = 0;
                APDS9930_enable_ps(APDS9930_i2c_client, enable);
            }
            else
            {
                APS_LOG("pockets ps dose not close\n");
            }
        }
    }
}
int check_ps_status_for_tp(void)
{
    int ret = -1;
    u16 psvalue = 0;
    if(apds9930_ps_enabled_for_tp && APDS9930_i2c_client != NULL)
    {
        if(APDS9930_read_ps(APDS9930_i2c_client, &psvalue) == 0)
        {
            APS_LOG("pockets ps value for tp:%d\n", psvalue);
            if(psvalue > ps_thd_val_high_for_tp)
            {
                APS_LOG("pockets ps status for tp: close\n");
                ret = 0;
            }
            else if(psvalue < ps_thd_val_low_for_tp)
            {
                APS_LOG("pockets ps status for tp: far away\n");
                ret = 1;
            }
        }
        else
        {
            APS_LOG("pockets ps status for tp: fail\n");
        }
    }
    else
    {
        APS_LOG("pockets ps status for tp: fail1\n");
    }
    return ret;
}
EXPORT_SYMBOL(APDS9930_enable_ps_for_tp);
EXPORT_SYMBOL(check_ps_status_for_tp);
/*added for tp pocket mode:end*/
...
/*singal interrupt function add*/
	...
	{
	    PS_CALILOG("ps intr auto calibration condition:intr_flag_value:%d[1:near.0:far]\n",intr_flag_value);
	    if(intr_ps_auto_calibration_flag == 0)
	    {
	        PS_CALILOG("ps intr auto calibration start...\n");
	        cali_ret = APDS9930_ps_auto_calibration(obj->client);
	        if(cali_ret == 0)
	        {
	            PS_CALILOG("ps intr auto calibration success\n");
	            intr_ps_auto_calibration_flag = 1;
	        }
	        else
	            PS_CALILOG("ps intr auto calibration fail\n");
	    }
	    else
	        PS_CALILOG("ps no need to calibration in intr\n");
	}
#endif			
...
static int APDS9930_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*added for tp pocket mode:start*/
	ps_thd_val_high_for_tp = obj->hw->ps_threshold_high;
	ps_thd_val_low_for_tp = obj->hw->ps_threshold_low;
	/*added for tp pocket mode:end*/
	obj->enable = 0;
	obj->pending_intr = 0;

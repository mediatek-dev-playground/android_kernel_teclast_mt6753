...
static kal_uint32 open(void)
{
    ...
    unsigned short golden_rg = 0x14E;
	unsigned short golden_bg = 0x143;
    LOG_1;
    LOG_2;
    ...
    sensor_init();
    mdelay(10);
   if(otp_update_wb(golden_rg,golden_bg))
		    LOG_INF("OV5648  OTP successed\n");
	 else
		    LOG_INF("OV5648  OTP Failed\n");
...
void otp_clear(void)
{
	...
	for (i=0; i<16; i++) 
	{
		write_cmos_sensor(OTP_DATA_ADDR+i, 0x00);
	}
}
  ...
  unsigned char  mid;
    	if (index > 2)
	{
   	...
   	else if (index < 2)
   	{
   		...
   		write_cmos_sensor(OTP_BANK, 0x00);
   		write_cmos_sensor(OTP_END_ADDR, 0x0f);	
   	}
   	else /*index==2*/
   	{
   		// select bank 1
   		write_cmos_sensor(OTP_BANK_ADDR, 0xc0);
   		write_cmos_sensor(OTP_BANK, 0x10);
   		write_cmos_sensor(OTP_END_ADDR, 0x1f);	
   		otp_addr = 0x3D07;
   	}
	...
	{
		mid = (flag&0x7f);
		LOG_INF("wb group %d has valid data\n", index);
		LOG_INF("This MID is 0x%x \n", mid);
		return 2;
	}
	else
	{
		LOG_INF("wb group %d has invalid data\n", index);
		return 1;
	}
}
...
signed char otp_read_wb_group(signed char index)
{
	...
	if (index == -1)
	{
		...
		for (index=0; index<3; index++)
		{
			...
			{
				LOG_INF("read wb from group %d", index);
				break;
			}
		}
	...
	else
	{
		if (otp_check_wb_group(index) != 2)
		{
			LOG_INF("read wb from group %d failed\n", index);
			return -1;
		}
	}

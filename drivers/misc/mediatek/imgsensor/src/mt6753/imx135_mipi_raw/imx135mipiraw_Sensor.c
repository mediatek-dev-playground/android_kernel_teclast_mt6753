...
#define SUNNY_OTP //zhangjiajuan,add for sunny otp,150410
...
bool IMX135_ReadOtp(int tempbank,USHORT address,BYTE* iBuffer,int buffersize)
{
	USHORT reVal;
	USHORT reg;
	BOOL res = TRUE;
	...
	{
		i=0;
		write_cmos_sensor(0x3b02, tempbank);
		write_cmos_sensor(0x3b00, 0x01);
		mdelay(100);
		reg = read_cmos_sensor(0x3b01);
		if ((reg&0x01) ==0x01)
		{
			LOG_INF("read flag in page %d success",tempbank);
		}
		else
		{
			LOG_INF("read flag in page %d failed",tempbank);
			return FALSE;
		}
		while(i<64)
		{
			reVal =read_cmos_sensor(address+i);
			*(iBuffer+k) = reVal;
			i++;
			k++;
			if (k>=buffersize)
...
void TestAWBforIMX135(USHORT RoverG_dec,USHORT BoverG_dec,USHORT GboverGr_dec)
{
	USHORT RoverG_dec_base,BoverG_dec_base,GboverGr_dec_base;
	USHORT R_test,B_test,G_test;
	USHORT R_test_H3,R_test_L8,B_test_H3,B_test_L8,G_test_H3,G_test_L8;
	RoverG_dec_base = 594;//the typcical value
	BoverG_dec_base = 589;//the typcical value
	USHORT G_test_R, G_test_B;
	...
	write_cmos_sensor(0x020E,G_test_H3);
	write_cmos_sensor(0x020F,G_test_L8);
	write_cmos_sensor(0x0210,R_test_H3);
	write_cmos_sensor(0x0211,R_test_L8);
	write_cmos_sensor(0x0212,B_test_H3);
	write_cmos_sensor(0x0213,B_test_L8);
	write_cmos_sensor(0x0214,G_test_H3);
	write_cmos_sensor(0x0215,G_test_L8);
}


//read module information and awb data
BOOL IMX135_ReadAWBFromOtp(void)
{
	USHORT awbGroupbank[] = {0x02,0x01};//the two fist group for AWB data
	USHORT address = 0x3b04;
	USHORT MID=0x01;
	USHORT LensID=0x01;
	USHORT VcmID=0x01;
	USHORT r,g,b;
	...
	for(i=0;i<2;i++)
	{
		...
		write_cmos_sensor(0x3b02,awbGroupbank[i]);//select page
		//turn on OTP read mode
		write_cmos_sensor(0x3b00,0x01);//read mode:0x3b00 = 0x01
		//check status(bit0:0x01 read ready)
		temp3 = read_cmos_sensor(0x3b01);//Status check : 0x3B01 = 0x01 (bit0 1:read ready)
		if ((temp3&0x01) ==0x01)
		{
			LOG_INF("read flag in page %d success",awbGroupbank[i]);//´òÓ¡
		}
		else
		{
			LOG_INF("read flag in page %d failed",awbGroupbank[i]);
			return FALSE;
		}
		//0x3b04-0x3b43 are the register address area for each page
		//check program flag by read ox3b04
		temp3 = read_cmos_sensor(0x3b04);
	...
	if (index == -1)
	{
		LOG_INF("No AWB data in OTP");
		return FALSE;
	}
	//2.read data from the valid group
	int tempbank = awbGroupbank[index];
	if(FALSE == IMX135_ReadOtp(tempbank,address,Temp,42))
	{
		LOG_INF("read page %d error",tempbank);
		return FALSE;
	}
	//3.check sum
	USHORT sum=0;
	...
	if (Temp[1] != (sum%256))
	{
		LOG_INF("AWB checksum error");
		return FALSE;
	}
	//4.check module information
	if(Temp[6]!=MID||Temp[7]!=LensID||Temp[8]!=VcmID)
	{
		LOG_INF("id error,mid=%d,lensid=%d,vcmid=%d",Temp[6],Temp[7],Temp[8]);
		return FALSE;
	}
	//4.get awb data
	r = (Temp[15]<<8)|Temp[16];
	...
	TestAWBforIMX135(r,b,g);
	return TRUE;
}


//read lsc data
BOOL IMX135_ReadLSCFromOtp(void)
{
	USHORT LscGroupbank[] = {0x09,0x03};//the two fist group for lsc data
	USHORT address = 0x3b04;
	int i;
	BYTE Temp[384]={0};
	USHORT temp3;
	...
	{
		//select OTP page address for read
		write_cmos_sensor(0x3b02,LscGroupbank[i]);
		//turn on OTP read mode
		write_cmos_sensor(0x3b00,0x01);
		//check status(bit0:0x01 read ready)
		temp3 = read_cmos_sensor(0x3b01);
		if ((temp3&0x01) ==0x01)
		{
			LOG_INF("read LSC flag on page %d success",LscGroupbank[i]);
		}
		else
		{
			LOG_INF("read LSC flag on page %d failed",LscGroupbank[i]);
			return FALSE;
		}
		//0x3b04-0x3b43 are the register address area for each page
		//check program flag by read ox3b04
	...
	if (index == -1)
	{
		LOG_INF("No LSC data in OTP");
		return FALSE;
	}
	//2.read data from the valid group
	int tempbank = LscGroupbank[index];
	if(FALSE == IMX135_ReadOtp(tempbank,address,Temp,384))
	{
		LOG_INF("read page %d error",tempbank);
		return FALSE;
	}
	...
	for(i=0;i<504;i+=4)
	{
		tempBuffer[i] = Temp[y]>>4;
		sum = sum+Temp[y];
		tempBuffer[i+1] = Temp[y+1];
		sum = sum+Temp[y+1];
		tempBuffer[i+2] = Temp[y]&0x0f;
		sum = sum+Temp[y+2];
		tempBuffer[i+3] = Temp[y+2];
		y+=3;
	}
	sum =sum+Temp[2];
	if (Temp[1] != (sum%256))
	{
		LOG_INF("LSC checksum error");
		return FALSE;
	}
	//4.write shading to sensor
	for(i=0;i<504;i++)
	{
		write_cmos_sensor(0X4800+i, tempBuffer[i]);
	}
	write_cmos_sensor(0x4500,0x1f);
	write_cmos_sensor(0x0700,0x01);
	write_cmos_sensor(0x3a63,0x01);
	return TRUE;
}


//the main functiion to call
BOOL OnReadOtpIMX135(void)
{
	// off lsc
	write_cmos_sensor(0x4500,0x0);
	write_cmos_sensor(0x0700,0x0);
	write_cmos_sensor(0x3a63,0x0);
	if (FALSE == IMX135_ReadLSCFromOtp())
	{
		return FALSE;
	}
	if (FALSE == IMX135_ReadAWBFromOtp())
	{
		return FALSE;
	}
	return TRUE;
...
#ifdef SUNNY_OTP
	if (TRUE != OnReadOtpIMX135())
	{
		LOG_INF("OTP update error\n");
	}
	else
	{
		LOG_INF("OTP update success\n");
...
{
#if 1 //#ifndef CONFIG_MTK_CAM_CAL

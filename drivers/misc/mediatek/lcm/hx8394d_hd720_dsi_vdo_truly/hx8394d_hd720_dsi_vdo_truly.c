...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.noncont_clock = TRUE; //add zhjg
...
#ifdef BUILD_LK
	printf("%s-%s LK debug: hx8394d id = 0x%08x\n", __func__, LCM_NAME, buffer);
#endif

	if (buffer == HX8394D_HD720_ID) 
	{
		char id_buf;
		data_array[0] = 0x00013700;
		dsi_set_cmdq(data_array, 1, 1);
...
#ifdef BUILD_LK
		printf("%s-%s LK debug: hx8394d idinfo = 0x%08x\n", __func__, LCM_NAME, id_buf);
#endif
		if (id_buf == 0x0D)
			return 1;
...
{
    .name			= "hx8394d_hd720_dsi_truly_vdo",
